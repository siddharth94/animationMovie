...
bool init()
{
    ...
    if( screen == NULL )
        return false;
    SDL_WM_SetCaption( "Thank You", NULL );
    if( TTF_Init() == -1 )
        return false;
...
bool load_files()
{
    ...
    background = load_image("jour.jpg");
...
int main( int argc, char* args[] )
{
    ...
    fps.start();
    message = TTF_RenderText_Solid( font, "Thank you!", textColor );
    apply_surface( 450, 100, message, screen );
    message = TTF_RenderText_Solid( font, "Kunal Kapila (14343)", textColor );
    apply_surface( 450, 150, message, screen );
    message = TTF_RenderText_Solid( font, "Siddharth Gupta (13695)", textColor );
    apply_surface( 450, 200, message, screen );
    if( SDL_Flip( screen ) == -1 )
        return 1;
    if( fps.get_ticks() < 1000 / FRAMES_PER_SECOND )
        SDL_Delay( ( 1000 / FRAMES_PER_SECOND ) - fps.get_ticks() );
